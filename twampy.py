...
##############################
SERVER = "192.168.111.220" # Zabbix server address

SERVER_USER = "Admin"
SERVER_PASSWORD = "zabbix"   # Just for tests, better use token for safety
# Or
TOKEN = ""


HOSTNAME = "sfp"                # Hostname by default (--host flag is not provided)
SESSION = "Classic"        # Session name by default (--session flag is not provided) if there is not session yet
DNS_NAME = ""                   # DNS device name


# Here you can type default S and R configs
IP_SENDER = ""
PORT_SENDER = ""
IP_RESPONDER = ""
PORT_RESPONDER = ""
...
##############################
HOSTGROUP_NAME = "twampy_sfp"
TEMPLATE_NAME = "twampy_sfp" # Template should be imported to zabbix
...
# Function to check if parameters match, excluding SESSIONID
def is_duplicate_session(session1, session2):
    return (session1["{#IPSENDER}"] == session2["{#IPSENDER}"] and
            session1["{#PORTSENDER}"] == session2["{#PORTSENDER}"] and
            session1["{#IPRESPONDER}"] == session2["{#IPRESPONDER}"] and
            session1["{#PORTRESPONDER}"] == session2["{#PORTRESPONDER}"])
...
class twampStatistics():
    ...
    def sendZabbix(self, total):

        sender = Sender(server=SERVER, port=10051)
        api = ZabbixAPI(url=SERVER, skip_version_check=True)
        if TOKEN:
            api.login(token=TOKEN)
        else:
            api.login(user=SERVER_USER, password=SERVER_PASSWORD)
        ...
        # Look for host with this interface
        hosts = api.host.get(
            output=["hostid", "name"],
            selectInterfaces=["ip"],
            filter={"ip": IP_SENDER}
        ...
        if hosts:
            ...
            global HOSTNAME
            HOSTNAME = hosts["name"]
            print(f'There is "{HOSTNAME}" host with this IP-address')
        else:
            print("There is no any host with this IP-address")
            ...
            if groups:
                ...
                print(f"Hostgroup {HOSTGROUP_NAME} was found")
            else:
                GROUP_ID = api.hostgroup.create(
                    name = HOSTGROUP_NAME
                )["groupids"][0]
                print(f"There is no group with this name. Hostgroup {HOSTGROUP_NAME} was created")

            if (api.host.get(
                filter = {"name": HOSTNAME}
            )):
                log.error(f"Hostname {HOSTNAME} already in use. Please change HOSTNAME")
                exit(1)

            HOST_ID = api.host.create(
                host = f"{HOSTNAME}",
                groups = [{"groupid": GROUP_ID}],
                templates = [{"templateid": TEMPLATE_ID}],
                description = f"S {IP_SENDER}:{PORT_SENDER}\nR {IP_RESPONDER}:{PORT_RESPONDER}",
                interfaces = [{
                    "type": 1, # Zabbix agent
                    "main": 1, # Default host's interfaces
                    "useip": 1,
                    "ip": IP_SENDER,
                    "dns": DNS_NAME,
                    "port": PORT_SENDER
                ...
                macros = [
                    {"macro": "{$IP.SENDER}", "value": IP_SENDER},
                    {"macro": "{$PORT.SENDER}", "value": PORT_SENDER},
                    {"macro": "{$IP.RESPONDER}", "value": IP_RESPONDER},
                    {"macro": "{$PORT.RESPONDER}", "value": PORT_RESPONDER},

                    {"macro": "{$RT.JITTER.WARN}", "value": "10", "description": "Warning threshold of roundtrip jitter in ms."},
                    {"macro": "{$RT.LATENCY.WARN}", "value": "20", "description": "Warning threshold of roundtrip latency in ms."},

                    {"macro": "{$ICMP_RESPONSE_TIME_WARN}", "value": "0.15", "description": "Warning threshold of the average ICMP response time in seconds."},
                    {"macro": "{$ICMP_LOSS_WARN}", "value": "20", "description": "Warning threshold of ICMP packet loss in %."}
                ],
                tags = [
                    {"tag": "S ip", "value": IP_SENDER},
                    {"tag": "S port", "value": PORT_SENDER},
                    {"tag": "R ip", "value": IP_RESPONDER},
                    {"tag": "R port", "value": PORT_RESPONDER}
                ]
            )
            print(f"Host {HOSTNAME} was created")
        ...
        current_session = {
            "{#SESSIONID}": SESSION,
            "{#IPSENDER}": IP_SENDER,
            "{#PORTSENDER}": PORT_SENDER,
            "{#IPRESPONDER}": IP_RESPONDER,
            "{#PORTRESPONDER}": PORT_RESPONDER
        }

        # Check for dublicate sessions
        session_updated = False
        for i, existing_session in enumerate(sessions):
            if existing_session == current_session:  # Полное совпадение, включая SESSIONID
                print(f"Found existing session: {current_session['{#SESSIONID}']} {current_session['{#IPSENDER}']}:{current_session['{#PORTSENDER}']} {current_session['{#IPRESPONDER}']}:{current_session['{#PORTRESPONDER}']}")
                session_updated = True
                break
            elif is_duplicate_session(existing_session, current_session):  # Совпадение параметров, но разный SESSIONID
                old_session_id = existing_session["{#SESSIONID}"]
                sessions[i]["{#SESSIONID}"] = current_session["{#SESSIONID}"]
                print(f"Update SessionID for existing session: {old_session_id} -> {current_session['{#SESSIONID}']} {current_session['{#IPSENDER}']}:{current_session['{#PORTSENDER}']} {current_session['{#IPRESPONDER}']}:{current_session['{#PORTRESPONDER}']}")
                session_updated = True
                break

        # Add sessions if it's new
        if not session_updated:
            sessions.append(current_session)
            print(f"Created new session: {current_session['{#SESSIONID}']} {current_session['{#IPSENDER}']}:{current_session['{#PORTSENDER}']} {current_session['{#IPRESPONDER}']}:{current_session['{#PORTRESPONDER}']}")


        # Form LLD JSON
        lld_data = {"data": sessions}
        # Translate JSON to JSON string
        lld_json_string = json.dumps(lld_data)
        sender.send_value(HOSTNAME, "session.discovery", lld_json_string)

        with open(JSON_FILENAME, "w") as f:
            json.dump(lld_data, f)

        print("===============================================================================")
        
        if self.count > 0:
            # Send values if there is anything to send
            self.lossRT = total - self.count
            items = [
                ItemValue(HOSTNAME, f"out.min[{SESSION}]", abs(self.minOB)),
                ItemValue(HOSTNAME, f"out.max[{SESSION}]", abs(self.maxOB)),
                ItemValue(HOSTNAME, f"out.avg[{SESSION}]", abs(self.sumOB / self.count)),
                ItemValue(HOSTNAME, f"out.jit[{SESSION}]", abs(self.jitterOB)),
                ItemValue(HOSTNAME, f"out.los[{SESSION}]", 100 * float(self.lossOB) / total),

                ItemValue(HOSTNAME, f"in.min[{SESSION}]", abs(self.minIB)),
                ItemValue(HOSTNAME, f"in.max[{SESSION}]", abs(self.maxIB)),
                ItemValue(HOSTNAME, f"in.avg[{SESSION}]", abs(self.sumIB / self.count)),
                ItemValue(HOSTNAME, f"in.jit[{SESSION}]", abs(self.jitterIB)),
                ItemValue(HOSTNAME, f"in.los[{SESSION}]", 100 * float(self.lossIB) / total),

                ItemValue(HOSTNAME, f"rt.min[{SESSION}]", abs(self.minRT)),
                ItemValue(HOSTNAME, f"rt.max[{SESSION}]", abs(self.maxRT)),
                ItemValue(HOSTNAME, f"rt.avg[{SESSION}]", abs(self.sumRT / self.count)),
                ItemValue(HOSTNAME, f"rt.jit[{SESSION}]", abs(self.jitterRT)),
                ItemValue(HOSTNAME, f"rt.los[{SESSION}]", 100 * float(self.lossRT) / total),

                ItemValue(HOSTNAME, f"twamp.status[{SESSION}]", 1)
            ]
            sender.send(items)
        else:
            # Set twamp status to zero if twampy proccess was failed
            sender.send_value(HOSTNAME, f"twamp.status[{SESSION}]", 0)
...
def twl_sender(args):
    global IP_SENDER, PORT_SENDER, IP_RESPONDER, PORT_RESPONDER, HOSTNAME, SESSION  # Declare globals to modify them

    # Parse near_end and far_end to extract IPs and ports
    sip, spt, sipv = parse_addr(args.near_end, 20000)    # Sender (local)
    rip, rpt, ripv = parse_addr(args.far_end, 20001)     # Responder (remote)

    # Update global variables with parsed values
    IP_SENDER = sip if sip else IP_SENDER  # Use existing value if sip is empty
    PORT_SENDER = str(spt)                 # Convert port to string
    IP_RESPONDER = rip if rip else IP_RESPONDER  # Use existing value if rip is empty
    PORT_RESPONDER = str(rpt)              # Convert port to string

    # Update HOSTNAME if --host is provided
    if args.host is not None:
        HOSTNAME = args.host
        log.info(f"Updated HOSTNAME to: {HOSTNAME}")

    if args.session is not None:
        SESSION = args.session
        log.info(f"Updated SESSION to: {SESSION}")

    log.info(f"Updated global variables: IP_SENDER={IP_SENDER}, PORT_SENDER={PORT_SENDER}, "
             f"IP_RESPONDER={IP_RESPONDER}, PORT_RESPONDER={PORT_RESPONDER}, HOSTNAME={HOSTNAME}, SESSION={SESSION}")
    print("===============================================================================")    

    # Proceed with creating and running the sender
    sender = twampySessionSender(args)
    sender.daemon = True
    sender.name = "twl_sender"
    sender.start()
...
if __name__ == '__main__':
    ...
    group.add_argument('--host', metavar='hostname', default=None, type=str, help='Custom hostname for Zabbix reporting')
    group.add_argument('--session', metavar='session name', default=None, type=str, help='Custom session name for Zabbix reporting')
